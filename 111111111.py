...
def determinant(self):
    ...
    for c in range(len(self.matrix)):
        m = Matrix.minor(self.matrix, 0, c)
        determinant += ((-1) ** c) * self.matrix[0][c] * (Matrix.determinant(m))
    return determinant


det = Matrix.determinant(self.matrix)
        if det == 0 or self.m != self.n:
            raise Exception("Несуществует обратной матрицы")
        if len(self.matrix) == 2:
            ...
            for r in range(self.n):
                ...
                for c in range(self.n):
                    ...
                    invr.append(((-1) ** (r + c)) *Matrix.determinant(minor))
                inv.append(invr)
            inv1 = Matrix.transpose(inv)
            for r in range(len(inv1)):
                for c in range(len(inv1)):
                    inv1[r][c] = inv1[r][c] / det
            result = Matrix(inv1)
            return result
...
def determinant(self):
    if isinstance(self, Matrix) & self.is_square():
        if len(self.matrix) == 1:
            ...
            det = 0
            for col, row in enumerate(self.matrix[0]):
                m = [i[:col] + i[col + 1:] for i in self.matrix[1:]]
                s = 1 if col % 2 == 0 else -1
                det += s * row * Matrix(m).determinant()
            return det
    else:
        raise Exception('The matrix in empty')
...
def inverse(self):
    ...
    for r in range(len(m)):
        ...
        for c in range(len(m)):
            minor_det = Matrix.determinant(Matrix.minor(self.matrix, r, c))
